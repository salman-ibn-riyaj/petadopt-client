...
const MyListingsPage = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5001/add-pet/${selectedPetId}`, {
        method: "DELETE",
      });
            ...
            >
              ...
              <div className="p-4 flex-1">
                ...
                <p className="text-xs text-pink-500 font-semibold mb-3">
                  {pet.species} • <span className="text-gray-500 dark:text-gray-400 font-normal">{pet.breed}</span>
                </p>
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-4 border-t border-gray-100 dark:border-[#30363d] pt-3 text-xs text-gray-600 dark:text-gray-300">
                  ...
                  <div className="flex items-center gap-1.5 col-span-2 text-[11px]">
                    ...
                    <span>Vaccinated: <strong className={pet.vaccinationStatus === "Yes" ? "text-emerald-500" : "text-amber-500"}>{pet.vaccinationStatus}</strong></span>
                  </div>
                </div>
              </div>
